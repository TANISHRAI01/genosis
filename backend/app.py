...
from langchain_community.chat_models import ChatOpenAI as ChatOpenAICompat
...
# Use Groq as free alternative to OpenAI
LLM_API_KEY = groq_api_key_str or openai_api_key_str
LLM_BASE_URL = "https://api.groq.com/openai/v1" if groq_api_key_str else None
LLM_MODEL = "llama-3.3-70b-versatile" if groq_api_key_str else "gpt-4o"
...
try:
    ...
    print("Video/audio/image processing via Vertex AI will be unavailable.")
...
if LLM_BASE_URL:
    vanna_config["base_url"] = LLM_BASE_URL

sqlBot = SQLBot(
    config={**vanna_config, "path": "./SQLVectorStore", "client": "persistent"}
)

csvBot = CSVBot(
    config={**vanna_config, "path": "./CSVVectorStore", "client": "persistent"}
)
...
# Reset the pdfs collection since we changed embedding model
try:
    persistent_client.delete_collection("pdfs")
except Exception:
    pass
persistent_client.get_or_create_collection("pdfs")
...
def openai_mindmap_generator(file: UploadFile, flow_id: str, flow_type: str):
    ...
    # Extract text content from the file based on type
    if file_extension == "csv":
        df = pd.read_csv(BytesIO(file_bytes))
        file_text_content = df.to_string()
    elif file_extension in ["txt", "html", "md"]:
        file_text_content = file_bytes.decode("utf-8", errors="replace")
        ...
        try:
            ...
            file_text_content = ""
            for page in pdf_doc:
                file_text_content += page.get_text()
            pdf_doc.close()
        except Exception:
            file_text_content = file_bytes.decode("utf-8", errors="replace")
    elif file_extension == "docx":
    ...
    max_chars = 60000
    if len(file_text_content) > max_chars:
        file_text_content = file_text_content[:max_chars] + "\n\n[... content truncated ...]"
    ...
    # Use regular chat completions API (works with Groq, OpenAI, etc.)
    client = openai.OpenAI(api_key=LLM_API_KEY, base_url=LLM_BASE_URL)
    response = client.chat.completions.create(
        model=LLM_MODEL,
        messages=[
            {"role": "system", "content": "Your task is to create the mindmap of the document. Return ONLY valid JSON."},
